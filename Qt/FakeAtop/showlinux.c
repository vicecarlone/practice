...
const char *
procprt_COMMAND_LINE_ae_mod(struct tstat *curstat, int avgval, int nsecs)
{
    ...
    sprintf(buf, "%.*s", CMDLEN, pline+curoffset);
#endif
    strcpy(curstat->gen.cmdline, buf);
}
...
extern int shmid;
#endif
int priproc(struct tstat **proclist, int firstproc, int lastproc, int curline,
            ...
            struct syscap *sb, int nsecs, int avgval)
{
    ...
    /*
        ...
        */
#ifndef _USE_NCURSES
    zatop_share_mem_t * zsm = shmat(shmid, NULL, 0);
    pthread_mutex_lock(&zsm->mutex);
#endif
    for (i=firstproc; i < lastproc; i++)
    {
        ...
        zsm->avgval = avgval;
        zsm->nsecs = nsecs;
        memcpy(&zsm->curstat[i], curstat, sizeof(struct tstat));
        zsm->num_proc++;
#else
        ...
        case MPROCNET:
        {
            int okbps = (curstat->net.tcpssz + curstat->net.udpssz)/125/nsecs;
            int ikbps = (curstat->net.tcprsz + curstat->net.udprsz)/125/nsecs;
            //if(okbps > 0 && ikbps > 0)
            //   printf("%d: %d -%s - %dkbps - %dkbps - %s\n", count_proc++, curstat->gen.pid, curstat->gen.name, okbps, ikbps, procprt_COMMAND_LINE_ae_mod(curstat, avgval, nsecs));
...
#ifndef _USE_NCURSES
    pthread_cond_signal(&zsm->cond);
    pthread_mutex_unlock(&zsm->mutex);
