...
#ifdef _USE_NCURSES
        int lenavail 	= (screen ? COLS : linelen) - 49 - seclen - utsnodenamelen;
#else
        int lenavail = linelen - 49 - seclen - utsnodenamelen;
#endif
        int len1	= lenavail / 3;
        ...
        */
        ...
        if (ncurlist > 0 && plistsz > 0)
        {
            ...
            */
            ...
            priphead(firstproc/plistsz+1, (ncurlist-1)/plistsz+1, &showtype, &curorder, showorder == MSORTAUTO ? 1 : 0);
            if (screen)
            {
                attroff(A_REVERSE);
                clrtobot();
