...
char addressParser(char addr[128]){
    int temp = 0, length = 0;
    char output[16];
    ...
    for (int i = 0; i < 8; i++){
        ...
        if(i%2) {temp = 0;
            sprintf(output+length*sizeof(char), "%d", temp);
    ...
    if (procinfo == NULL){
        ...
        do {
            if (fgets(buffer, sizeof(buffer), procinfo)){
                ...
                printf("Local addr: %s:%d \t Inode: %ld \t Process: %s \t State: %s\n",local_addr,local_port,inode,prg_cache_get(inode),tcp_state[state]);
            }
        } while (!feof(procinfo));
        fclose(procinfo);
    }
